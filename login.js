...
export function renderLogin(app, navigate, params = {}) {
    ...
    <div class="login-page">
      <div class="login-card">
        <div class="login-logo">
          <span class="material-symbols-rounded icon" style="${isTargetAdmin ? 'color:var(--accent-purple); text-shadow:0 0 15px var(--accent-purple-dim)' : ''}">
            ${isTargetAdmin ? 'admin_panel_settings' : 'sports_cricket'}
          </span>
          <h1>local Crickret App</h1>
          <p>${isTargetAdmin ? 'Tournament Administrator Hub' : 'Tournament Management System'}</p>
          ${isTargetAdmin ? '<div style="margin-top:10px"><span class="badge" style="background:var(--accent-purple); color:var(--bg-primary);">Admin Verification</span></div>' : ''}
        </div>
  ...
  document.getElementById('loginForm').addEventListener('submit', (e) => {
    ...
    if (isTargetAdmin) {
      if (user.role === 'admin') {
        showToast(`Welcome back Administrator, ${user.fullName}!`, 'success');
      } else {
        const verifyCode = prompt('Accessing the Admin Portal requires authentication. Please enter the admin verification passcode (2011):');
        if (verifyCode === '2011') {
          store.setAdminMode(true);
          showToast(`Admin access granted. Welcome, ${user.fullName}!`, 'success');
        } else {
          showToast('Incorrect passcode. Accessing as standard user.', 'warning');
  ...
  document.getElementById('googleSignIn').addEventListener('click', async () => {
    ...
    try {
      ...
      if (isTargetAdmin && appUser.role !== 'admin') {
        const verifyCode = prompt('Accessing the Admin Portal requires authentication. Please enter the admin verification passcode (2011):');
        if (verifyCode === '2011') {
          store.setAdminMode(true);
          showToast(`Admin access granted. Welcome, ${appUser.fullName}!`, 'success');
        } else {
          showToast('Incorrect passcode. Accessing as standard user.', 'warning');
          store.setAdminMode(false);
        }
