...
export function renderDashboard(app, navigate) {
    ...
    app.innerHTML = `
      <div class="dashboard-page animate-fade-in">
        <div class="dashboard-header">
          <div>
            <h1>Seasons Framework ${isAdmin ? '(Admin Dashboard)' : ''}</h1>
          ...
          <div style="display:flex;gap:var(--sp-2);align-items:center;flex-shrink:0">
            ...
            <div style="position:relative">
              ...
              <div class="user-dropdown hidden" id="userDropdown" style="position:absolute;right:0;top:44px;background:var(--bg-modal);border:1px solid var(--bg-card-border);border-radius:var(--radius-md);padding:var(--sp-2);min-width:180px;z-index:100;box-shadow:var(--shadow-lg)">
                ...
                ${!isAdmin
                  ? `<button class="btn btn-ghost btn-sm" id="adminModeBtn" style="width:100%;justify-content:flex-start;gap:8px">${icon('admin_panel_settings', 16)} Enter Admin Mode</button>`
                  : `<button class="btn btn-ghost btn-sm" id="exitAdminBtn" style="width:100%;justify-content:flex-start;gap:8px">${icon('person', 16)} Exit Admin Mode</button>`
                }
  ...
  function showAdminPrompt() {
    ...
    modalRoot.innerHTML = `
      <div class="modal-overlay" id="adminModal">
        <div class="modal-content" style="max-width:360px">
          <div class="modal-title">${icon('admin_panel_settings', 24)} Admin Access</div>
          <button class="modal-close" id="closeAdminModal">${icon('close')}</button>
          <div class="admin-prompt">
            <p style="color:var(--text-muted);font-size:var(--fs-sm);margin-bottom:var(--sp-4)">Enter admin password to enable admin mode.</p>
            <div class="form-group">
              <div class="input-password-wrap">
                <input type="password" id="adminPwdInput" class="form-input" placeholder="Admin password" autocomplete="off" />
              </div>
            </div>
            <div class="modal-actions" style="justify-content:center">
              <button class="btn btn-outline" id="cancelAdmin">Cancel</button>
              <button class="btn btn-primary" id="confirmAdmin">Verify</button>
            </div>
          </div>
        </div>
      </div>
    ...
    document.getElementById('closeAdminModal').addEventListener('click', () => modalRoot.innerHTML = '');
    document.getElementById('cancelAdmin').addEventListener('click', () => modalRoot.innerHTML = '');
    ...
    document.getElementById('adminPwdInput').addEventListener('keydown', (e) => {
      if (e.key === 'Enter') document.getElementById('confirmAdmin').click();
    });
  }
